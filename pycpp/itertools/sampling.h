    ...
    typename std::iterator_traits<BidirIter>::difference_type d = 0)
{
    ...
    if (d1 == 1) {
        ...
        std::swap(*first1, *std::prev(last2));
    ...
    if (d1 == 1) {
        std::swap(*std::prev(last2), *first3);
    }
    if (d != 0) {
        if (d2 > 1) {
    ...
    Function &f)
{
    switch (d1) {
        ...
        case 2:
            ...
            std::swap(*first1, *std::next(first1));
            return f();
        case 3: {
            if (f()) {
                return true;
    ...
    Function f)
{
    ...
    combine_discontinuous(first, mid, std::distance(first, mid), mid, last, std::distance(mid, last), wfunc);
    return std::move(f);
}
    ...
    Function f)
{
    ...
    combine_discontinuous(first, mid, d1, mid, last, std::distance(mid, last), pf);
    return std::move(f);
}
...
template <typename BidirIter>
std::uintmax_t count_each_permutation(BidirIter first,
    BidirIter mid,
    BidirIter last)
{
...
Function for_each_circular_permutation(BidirIter first,
    ...
    Function f)
{
    for_each_combination(first, mid, last, circular_permutation<Function&, BidirIter>(f, std::distance(first, mid)));
    return std::move(f);
}
...
template <typename BidirIter>
std::uintmax_t count_each_circular_permutation(BidirIter first,
    BidirIter mid,
    BidirIter last)
{
...
Function for_each_reversible_circular_permutation(BidirIter first,
    ...
    Function f)
{
    for_each_combination(first, mid, last, reverse_circular_permutation<Function&, BidirIter>(f, std::distance(first, mid)));
    return std::move(f);
}
