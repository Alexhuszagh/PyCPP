...
static constexpr size_t WHIRLPOOL_HASH_SIZE = 64;
static const uint64_t WHIRLPOOL_SBOX[8][256] = {
    {
...
{
    uint64_t length;
    uint8_t message[WHIRLPOOL_HASH_SIZE];
    uint64_t hash[8];
};
...
static void whirlpool_process_block(uint64_t* hash, uint64_t* block)
{
    ...
    for (i = 0; i < 8; i++) {
        ...
        k[0][i] = hash[i];
        state[0][i] = be64toh(block[i]) ^ hash[i];
        hash[i] = state[0][i];
    }
...
static void whirlpool_update(whirlpool_context *ctx, const uint8_t* msg, size_t size)
{
    ...
    while (size >= WHIRLPOOL_HASH_SIZE) {
        ...
        if (IS_ALIGNED_64(msg)) {
            ...
            memcpy(ctx->message, msg, WHIRLPOOL_HASH_SIZE);
...
size_t whirlpool_hash::hexdigest(void* dst, size_t dstlen) const
{
    ...
    return hex_i8(hash, WHIRLPOOL_HASH_SIZE, dst, dstlen);
}
