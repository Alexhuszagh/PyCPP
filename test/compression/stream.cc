//  :copyright: (c) 2017 Alex Huszagh.
//  :license: MIT, see LICENSE.md for more details.
/*
 *  \addtogroup Tests
 *  \brief Filtering compression stream unittests.
 */

#include <pycpp/compression/stream.h>
#include <pycpp/stl/sstream.h>
#include <gtest/gtest.h>
#if BUILD_FILESYSTEM
#   include <pycpp/filesystem.h>
#endif          // BUILD_FILESYSTEM

PYCPP_USING_NAMESPACE

// DATA
// ----

static string BZ2_COMPRESSED("", 710);
static string ZLIB_COMPRESSED("", 669);
static string LZMA_COMPRESSED("", 756);
static string GZIP_COMPRESSED("", 689);
static string DECOMPRESSED("", 1110);

// HELPERS
// -------

#define STREAM_WRAPPER(name)                            \
    struct name##_istream_wrapper: name##_istream       \
    {                                                   \
        using name##_istream::name##_istream;           \
        using name##_istream::swap;                     \
    };                                                  \
                                                        \
    struct name##_ostream_wrapper: name##_ostream       \
    {                                                   \
        using name##_ostream::name##_ostream;           \
        using name##_ostream::swap;                     \
    }


template <typename IStream>
struct test_istream
{
    template <typename String>
    void standard(const String& input)
    {
        istringstream sstream(input);
        IStream ifs(sstream);
        ostringstream ofs;
        ofs << ifs.rdbuf();

        EXPECT_EQ(ofs.str(), DECOMPRESSED);
    }

    template <typename String>
    void move(const String& input)
    {
        istringstream sstream(input);
        IStream ifs1(sstream), ifs2;
        ifs1.swap(ifs2);
        ostringstream ofs;
        ofs << ifs2.rdbuf();

        EXPECT_EQ(ofs.str(), DECOMPRESSED);
    }

    template <typename String>
    void operator()(const String& input)
    {
        standard(input);
        move(input);
    }
};


template <typename OStream>
struct test_ostream
{
    template <typename String>
    void standard(const String& output, bool check)
    {
        ostringstream sstream;
        {
            OStream ofs(sstream);
            ofs << DECOMPRESSED;
        }
        if (check) {
            EXPECT_EQ(sstream.str(), output);
        }
    }

    template <typename String>
    void move(const String& output, bool check)
    {
        ostringstream sstream;
        {
            OStream ofs1(sstream), ofs2;
            ofs1.swap(ofs2);
            ofs2 << DECOMPRESSED;
        }
        if (check) {
            EXPECT_EQ(sstream.str(), output);
        }
    }

    template <typename String>
    void operator()(const String& output, bool check = true)
    {
        standard(output, check);
        move(output, check);
    }
};


#if BUILD_FILESYSTEM

template <typename IStream>
struct test_ifstream
{
    template <typename String>
    void standard(const String& input)
    {
        string path("sample_input_file.txt");
        {
            ofstream ofs(path);
            ofs << input;
        }
        ostringstream ofs;
        {
            IStream ifs(path);
            ofs << ifs.rdbuf();
        }

        EXPECT_EQ(ofs.str(), DECOMPRESSED);
        EXPECT_TRUE(remove_file(path));
    }

    template <typename String>
    void move(const String& input)
    {
        string path("sample_input_file.txt");
        {
            ofstream ofs(path);
            ofs << input;
        }
        ostringstream ofs;
        {
            IStream ifs1(path), ifs2;
            ifs1.swap(ifs2);
            ofs << ifs2.rdbuf();
        }

        EXPECT_EQ(ofs.str(), DECOMPRESSED);
        EXPECT_TRUE(remove_file(path));
    }

    template <typename String>
    void operator()(const String& input)
    {
        standard(input);
        move(input);
    }
};


template <typename OStream>
struct test_ofstream
{
    template <typename String>
    void standard(const String& output, bool check)
    {
        string path("sample_output_file.txt");
        {
            OStream ofs(path);
            ofs << DECOMPRESSED;
        }
        if (check) {
            ifstream ifs(path);
            ostringstream sstream;
            sstream << ifs.rdbuf();
            EXPECT_EQ(sstream.str(), output);
        }

        EXPECT_TRUE(remove_file(path));
    }

    template <typename String>
    void move(const String& output, bool check)
    {
        string path("sample_output_file.txt");
        {
            OStream ofs(path);
            ofs << DECOMPRESSED;
        }
        if (check) {
            ifstream ifs1(path), ifs2;
            ifs1.swap(ifs2);
            ostringstream sstream;
            sstream << ifs2.rdbuf();
            EXPECT_EQ(sstream.str(), output);
        }

        EXPECT_TRUE(remove_file(path));
    }

    template <typename String>
    void operator()(const String& output, bool check = false)
    {
        standard(output, check);
        move(output, check);
    }
};

#endif          // BUILD_FILESYSTEM

// TESTS
// -----

#if defined(HAVE_BZIP2)

STREAM_WRAPPER(bz2);

TEST(compression_stream, bz2_istream)
{
    test_istream<bz2_istream_wrapper>()(BZ2_COMPRESSED);
#if BUILD_FILESYSTEM
    test_ifstream<bz2_ifstream>()(BZ2_COMPRESSED);
#endif          // BUILD_FILESYSTEM
}


TEST(compression_stream, bz2_ostream)
{
    test_ostream<bz2_ostream_wrapper>()(BZ2_COMPRESSED);
#if BUILD_FILESYSTEM
    test_ofstream<bz2_ofstream>()(BZ2_COMPRESSED);
#endif          // BUILD_FILESYSTEM
}

#endif                  // HAVE_BZIP2

#if defined(HAVE_ZLIB)

STREAM_WRAPPER(zlib);
STREAM_WRAPPER(gzip);

TEST(compression_stream, zlib_istream)
{
    test_istream<zlib_istream_wrapper>()(ZLIB_COMPRESSED);
#if BUILD_FILESYSTEM
    test_ifstream<zlib_ifstream>()(ZLIB_COMPRESSED);
#endif          // BUILD_FILESYSTEM
}


TEST(compression_stream, zlib_ostream)
{
    test_ostream<zlib_ostream_wrapper>()(ZLIB_COMPRESSED);
#if BUILD_FILESYSTEM
    test_ofstream<zlib_ofstream>()(ZLIB_COMPRESSED);
#endif          // BUILD_FILESYSTEM
}


TEST(compression_stream, gzip_istream)
{
    test_istream<gzip_istream_wrapper>()(GZIP_COMPRESSED);
#if BUILD_FILESYSTEM
    test_ifstream<gzip_ifstream>()(GZIP_COMPRESSED);
#endif          // BUILD_FILESYSTEM
}


TEST(compression_stream, gzip_ostream)
{
    test_ostream<gzip_ostream_wrapper>()(GZIP_COMPRESSED, false);
#if BUILD_FILESYSTEM
    test_ofstream<gzip_ofstream>()(GZIP_COMPRESSED, false);
#endif          // BUILD_FILESYSTEM
}

#endif                  // HAVE_ZLIB

#if defined(HAVE_LZMA)

STREAM_WRAPPER(lzma);

TEST(compression_stream, lzma_istream)
{
    test_istream<lzma_istream_wrapper>()(LZMA_COMPRESSED);
#if BUILD_FILESYSTEM
    test_ifstream<lzma_ifstream>()(LZMA_COMPRESSED);
#endif          // BUILD_FILESYSTEM
}


TEST(compression_stream, lzma_ostream)
{
    test_ostream<lzma_ostream_wrapper>()(LZMA_COMPRESSED);
#if BUILD_FILESYSTEM
    test_ofstream<lzma_ofstream>()(LZMA_COMPRESSED);
#endif          // BUILD_FILESYSTEM
}

#endif                  // HAVE_LZMA

TEST(compression_stream, decompressing_istream)
{
    // declare variables
    ostringstream ostream;
    istringstream sstream;

#if defined(HAVE_BZIP2)
    // bzip2
    ostream = ostringstream();
    sstream = istringstream(BZ2_COMPRESSED);
    {
        decompressing_istream compressed(sstream);
        ostream << compressed.rdbuf();
    }
    EXPECT_EQ(ostream.str(), DECOMPRESSED);
#endif                  // HAVE_BZIP2

#if defined(HAVE_ZLIB)
    // zlib
    ostream = ostringstream();
    sstream = istringstream(ZLIB_COMPRESSED);
    {
        decompressing_istream compressed(sstream);
        ostream << compressed.rdbuf();
    }
    EXPECT_EQ(ostream.str(), DECOMPRESSED);

    // gzip
    ostream = ostringstream();
    sstream = istringstream(GZIP_COMPRESSED);
    {
        decompressing_istream compressed(sstream);
        ostream << compressed.rdbuf();
    }
    EXPECT_EQ(ostream.str(), DECOMPRESSED);
#endif                  // HAVE_ZLIB

#if defined(HAVE_LZMA)
    // lzma
    ostream = ostringstream();
    sstream = istringstream(LZMA_COMPRESSED);
    {
        decompressing_istream compressed(sstream);
        ostream << compressed.rdbuf();
    }
    EXPECT_EQ(ostream.str(), DECOMPRESSED);
#endif                  // HAVE_LZMA
}
